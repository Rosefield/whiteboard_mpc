...
public:
	...
	TP* pool;
	bool *value;
	std::unordered_map<int, block *> key;
	std::unordered_map<int, block *> mac;
	NetIOMP<IO> * io;
	block Delta;
	...
	std::unordered_map<int, AuthBitShare> authenticated_share_assignment; // if this bit is from authenticated shares, the authenticated share is stored here

	FlexIn(int len, int nP, int party) {
		this->len = len;
                this->nP = nP;
		this->party = party;
		this->pool = pool;
	...
	void associate_cmpc(TP *associated_pool, bool *associated_value, std::unordered_map<int, block *>& associated_mac, std::unordered_map<int, block *>& associated_key,  NetIOMP<IO> *associated_io, block associated_Delta) {
	...
	void input(bool *masked_input_ret) {
		...
		for(int i = 0; i < len; i++) {
			...
			for(int j = 1; j <= nP; j++) {
				if(j != party) {
					abit.key[j-1] = key[j][i];
					abit.mac[j-1] = mac[j][i];
				}
			}
				...
				res_check.push_back(pool->enqueue([this, &input_mask, &open_bit_shares_for_plaintext_input_recv, j]() {
					for (int i = 0; i < len; i++) {
						if (party_assignment[i] == party) {
							...
							supposed_mac ^= input_mask[i].key[j-1];
		...
		for(int i = 0; i < len; i++) {
			AuthBitShare new_entry;
                        new_entry.key.resize(nP);
                        new_entry.mac.resize(nP);
			if(party_assignment[i] == -1) {
				new_entry.bit_share = authenticated_share_assignment[i].bit_share ^ input_mask[i].bit_share;
				for (int j = 1; j <= nP; j++) {
					new_entry.key[j-1] = authenticated_share_assignment[i].key[j-1] ^ input_mask[i].key[j-1];
					new_entry.mac[j-1] = authenticated_share_assignment[i].mac[j-1] ^ input_mask[i].mac[j-1];
				}
			}
		...
		for(int i = 0; i < 10; i++){
			if(party_assignment[i] == -1) {
				...
				for(int j = 1; j <= nP; j++) {
					if(j != party) {
						//cout << j << ": ";
						print_block(authenticated_share_assignment[i].mac[j-1]);
					}
				}
				cout << "key: " << endl;
				for(int j = 1; j <= nP; j++) {
					if(j != party) {
						//cout << j << ": ";
						print_block(authenticated_share_assignment[i].key[j-1]);
						print_block(authenticated_share_assignment[i].key[j-1] ^ Delta);
					}
				}
				cout << "=============" << endl;
			}
			...
			if(j != party) {
				for(int i = 0; i < len; i++) {
					...
					if(party_assignment[i] == -1) {
						...
						mbit.mac = authenticated_bitshares_new_circuit[i].mac[j-1];
					}
				...
				res_check.push_back(pool->enqueue([this, &authenticated_bitshares_new_circuit, &open_bit_shares_for_authenticated_bits_recv, j]() {
					for (int i = 0; i < len; i++) {
						if (party_assignment[i] == -1) {
							...
							supposed_mac ^= authenticated_bitshares_new_circuit[i].key[j-1];
...
public:
	...
	std::unordered_map<int, block*> key;
	std::unordered_map<int, block*> mac;
	std::unordered_map<int, block*> eval_labels;
	NetIOMP<IO> * io;
	block Delta;
	block *labels;
	...
	std::unordered_map<int, AuthBitShare> authenticated_share_results; // if this bit is from authenticated shares, the authenticated share is stored here

	FlexOut(int len, int nP, int party) {
		this->len = len;
                this->nP = nP;
		this->party = party;
		this->pool = pool;
	...
	void associate_cmpc(TP *associated_pool, bool *associated_value, std::unordered_map<int, block *>& associated_mac, std::unordered_map<int, block *>& associated_key, std::unordered_map<int, block *>& associated_eval_labels, block *associated_labels, NetIOMP<IO> *associated_io, block associated_Delta) {
		this->cmpc_associated = true;
		this->pool = associated_pool;
		this->value = associated_value;
	...
	void assign_party(int pos, int which_party) {
		party_assignment[pos] = which_party;
                ...
                if (which_party == -1) {
                    ...
                    abit.key.resize(nP);
                    abit.mac.resize(nP);
                    authenticated_share_results[pos] = abit;
                }
	}
		...
		if(party == ALICE) {
			for(int i = 0; i < len; i++) {
				if(party_assignment[i] == -1) {
					...
					for(int j = 1; j <= nP; j++) {
						if(j != party) {
							authenticated_share_results[i].mac[j-1] = mac[j][output_shift + i];
							authenticated_share_results[i].key[j-1] = key[j][output_shift + i];
						}
					}
				}
			}
				...
				if(party_assignment[i] == -1) {
					...
					for(int j = 1; j <= nP; j++) {
						if(j != party) {
							authenticated_share_results[i].mac[j-1] = mac[j][output_shift + i];
							if(j == ALICE) {
								authenticated_share_results[i].key[j-1] =
		...
		for(int i = 0; i < 10; i++){
			if(party_assignment[i] == -1) {
				...
				for(int j = 1; j <= nP; j++) {
					if(j != party) {
						//cout << j << ": ";
						print_block(authenticated_share_results[i].mac[j-1]);
					}
				}
				cout << "key: " << endl;
				for(int j = 1; j <= nP; j++) {
					if(j != party) {
						//cout << j << ": ";
						print_block(authenticated_share_results[i].key[j-1]);
						print_block(authenticated_share_results[i].key[j-1] ^ Delta);
					}
				}
				cout << "=============" << endl;
			}
