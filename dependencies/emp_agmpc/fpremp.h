...
class FpreMP { public:
	...
	PRG prg;
	int ssp;
	FpreMP(NetIOMP<IO> * io[2], TP * pool, int nP, int party, bool * _delta = nullptr, int ssp = 40) {
		this->party = party;
		this->pool = pool;
		this->io = io[0];
		this->ssp = ssp;
                this->nP = nP;
		abit = new ABitMP<TP, IO>(io[1], pool, nP, party, _delta, ssp);
		Delta = abit->Delta;
		prps = new CRH[nP+1];
		prps2 = new CRH[nP+1];
		prgs = new PRG[nP+1];
	...
	void compute(std::unordered_map<int, block*>& MAC, std::unordered_map<int, block*>& KEY, bool * r, int length) {
		int64_t bucket_size = get_bucket_size(length);
		std::unordered_map<int, block *> tMAC;
		std::unordered_map<int, block *> tKEY;
		block * tKEYphi[nP+1];
		block * tMACphi[nP+1];
		block * phi;
		block *X [nP+1];
		...
		for(int i = 1; i <= nP; ++i) for(int j = 1; j <= nP; ++j) if (i < j ) {
			if(i == party) {
                                auto tkj = tKEY[j];
                                auto sj = s[j];
				res.push_back(pool->enqueue([this, tkj, tr, sj, length, bucket_size, j]() {
					prgs[j].random_bool(sj, length*bucket_size);
                                        uint8_t * data = new uint8_t[length*bucket_size];
					for(int k = 0; k < length*bucket_size; ++k) {
						data[k] = garble(tkj, tr, sj, k, j);
						sj[k] = (sj[k] != (tr[3*k] and tr[3*k+1]));
					}

                                        io->send_data(j, data, length*bucket_size);
                                        delete[] data;
					io->flush(j);
				}));
			} else if (j == party) {
                                auto tmi = tMAC[i];
                                auto si = s[i];
				res.push_back(pool->enqueue([this, tmi, tr, si, length, bucket_size, i]() {
                                        uint8_t * data = new uint8_t[length*bucket_size];
                                        io->recv_data(i, data, length*bucket_size);
					for(int k = 0; k < length*bucket_size; ++k) {
						bool tmp = evaluate(data[k], tmi, tr, k, i);
						si[k] = (tmp != (tr[3*k] and tr[3*k+1]));
					}
                                        delete[] data;
				}));
			}
		...
		for(int i = 1; i <= nP; ++i) for(int j = 1; j<= nP; ++j) if( (i < j) and (i == party or j == party) ) {
			...
			res.push_back(pool->enqueue([this, e, length, bucket_size, party2]() {
				...
				io->flush(party2);
                        ...
                        auto tkp = tKEY[party2];
			res.push_back(pool->enqueue([this, tkp, length, bucket_size, party2]() {
				bool * tmp = new bool[length*bucket_size];
				io->recv_data(party2, tmp, length*bucket_size);
				for(int k = 0; k < length*bucket_size; ++k) {
					if(tmp[k])
						tkp[3*k+2] = tkp[3*k+2] ^ Delta;
				}
				delete[] tmp;
			}));
		}
...
#endif		
		auto f = abit->check(tMAC, tKEY, tr, length*bucket_size*3 + 3*ssp);
                f.get();
		...
		for(int i = 1; i <= nP; ++i) for(int j = 1; j<= nP; ++j) if( (i < j) and (i == party or j == party) ) {
			int party2 = i + j - party;
                        ...
                        auto tkp = tKEY[party2];
                        auto tkphip = tKEYphi[party2];
			res.push_back(pool->enqueue([this, tkp, tkphip, phi, length, bucket_size, party2]() {
				block bH[2], tmpH[2];
				for(int k = 0; k < length*bucket_size; ++k) {
					bH[0] = tkp[3*k];
					bH[1] = bH[0] ^ Delta;
				...
				io->flush(party2);
                        ...
                        auto tmp = tMAC[party2];
                        auto tmphip = tMACphi[party2];
			res.push_back(pool->enqueue([this, tmp, tmphip, tr, length, bucket_size, party2]() {
				block bH;
				for(int k = 0; k < length*bucket_size; ++k) {
					io->recv_data(party2, &bH, sizeof(block));
					block hin = sigma(tmp[3*k]) ^ makeBlock(0, 2*k+tr[3*k]);
					tmphip[k] = prps2[party2].H(hin);
					if(tr[3*k]) tmphip[k] = tmphip[k] ^ bH;
				}
			}));
		}
		joinNclean(res);
		...
		block prg_key = sampleRandom<TP, IO>(io, &prg, pool, party);
		PRG prgf(&prg_key);
		...
		for(int i = 1; i <= nP; ++i) for(int j = 1; j<= nP; ++j) if( (i < j) and (i == party or j == party) ) {
			...
			res2.push_back(pool->enqueue([this, xp, xp2, dgst, party2]() -> bool {
				io->send_data(party2, xp, sizeof(block)*ssp);
				io->recv_data(party2, xp2, sizeof(block)*ssp);
				char tmp[Hash::DIGEST_SIZE];
				Hash::hash_once(tmp, xp2, sizeof(block)*ssp);
		...
		//land -> and	
		block S = sampleRandom<TP>(io, &prg, pool, party);
		...
		for(int i = 1; i <= nP; ++i) for(int j = 1; j<= nP; ++j) if( (i < j) and (i == party or j == party) ) {
			int party2 = i + j - party;
                        auto dp = d[party];
			res.push_back(pool->enqueue([this, dp, length, bucket_size, party2]() {
				io->send_data(party2, dp, (bucket_size-1)*length);
				io->flush(party2);
			}));
                        auto dp2 = d[party2];
			res.push_back(pool->enqueue([this, dp2, length, bucket_size, party2]() {
				io->recv_data(party2, dp2, (bucket_size-1)*length);
			}));
		}
		joinNclean(res);
		for(int i = 2; i <= nP; ++i)
