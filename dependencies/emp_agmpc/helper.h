...
block sampleRandom(NetIOMP<IO> * io, PRG * prg, TP * pool, int party) {
...
void check_MAC(NetIOMP<IO> * io, block ** MAC, block ** KEY, bool * r, block Delta, int length, int party) {
...
template<typename IO>
void check_correctness(NetIOMP<IO>* io, bool * r, int length, int party) {
        auto nP = io->nP;
	if (party == 1) {
