...
#define GTX_idx(i,j,k) ((i)*(4*(nP+1)) + (j)*(nP+1) + k)
#define GT_idx(i,j,k,l) ((i)*((nP+1)*4*(nP+1)) + (j)*(4*(nP+1)) + (k)*(nP+1) + (l))

template<typename TP, typename IO>
class CMPC { public:
	const block MASK = makeBlock(0x0ULL, 0xFFFFFULL);
	FpreMP<TP, IO>* fpre = nullptr;
	...
	std::unordered_map<int, block*> preprocess_mac;
	std::unordered_map<int, block*> preprocess_key;
	bool* preprocess_value;

	std::unordered_map<int, block*> sigma_mac;
	std::unordered_map<int, block*> sigma_key;
	bool * sigma_value;

	std::unordered_map<int, block*> ANDS_mac;
	std::unordered_map<int, block*> ANDS_key;
	bool * ANDS_value;
	...
	block *GTK;
	bool (*GTv)[4];
        // array of [][nP+1][4][nP+1] 
	block *GT;
	std::unordered_map<int, block*> eval_labels;
	PRP prp;
	CMPC(NetIOMP<IO> * io[2], TP * pool, int nP, int party, BristolFormat * cf, bool * _delta = nullptr, int ssp = 40) {
                this->nP = nP;
		this->party = party;
		this->io = io[0];
		this->cf = cf;
		...
		fpre = new FpreMP<TP, IO>(io, pool, nP, party, _delta, ssp);
		Delta = fpre->Delta;
	...
	void function_independent() {
		...
		auto f = fpre->abit->check(preprocess_mac, preprocess_key, preprocess_value, total_pre);
                f.get();
...
//		ret.get();
	...
	void function_dependent() {
		...
		for(int i = 1; i <= nP; ++i) for(int j = 1; j <= nP; ++j) if( (i < j) and (i == party or j == party) ) {
			int party2 = i + j - party;
                        auto xp = x[party];
                        auto yp = y[party];
			res.push_back(pool->enqueue([this, xp, yp, party2]() {
				io->send_data(party2, xp, num_ands);
				io->send_data(party2, yp, num_ands);
				io->flush(party2);
			}));
                        auto xp2 = x[party2];
                        auto yp2 = y[party2];
			res.push_back(pool->enqueue([this, xp2, yp2, party2]() {
				io->recv_data(party2, xp2, num_ands);
				io->recv_data(party2, yp2, num_ands);
			}));
		}
		joinNclean(res);
		...
		if(party != 1) { 
			for(int i = 0; i < cf->num_gate; ++i) if(cf->gates[4*i+3] == AND_GATE) {
				...
				Hash((block*)H, labels[cf->gates[4*i]], labels[cf->gates[4*i+1]], ands);
				for(int j = 0; j < 4; ++j) {
			...
			for(int i = 2; i <= nP; ++i) {
				...
				res.push_back(pool->enqueue([this, party2]() {
					for(int i = 0; i < num_ands; ++i)
						for(int j = 0; j < 4; ++j)
							io->recv_data(party2, &GT[GT_idx(i, party2, j, 1)], sizeof(block)*(nP));
				}));
			}
			for(int i = 0; i < cf->num_gate; ++i) if(cf->gates[4*i+3] == AND_GATE) {
				r[0] = sigma_value[ands] != value[cf->gates[4*i+2]];
				...
				memcpy(&GTK[GTX_idx(ands, 0, 0)], K, sizeof(block)*4*(nP+1));
				memcpy(&GTM[GTX_idx(ands, 0, 0)], M, sizeof(block)*4*(nP+1));
		...
		if(party!= 1) {
			...
			for(int i = 0; i < cf->num_gate; ++i) {
				if (cf->gates[4*i+3] == XOR_GATE) {
					...
					block H[nP+1];
					for(int j = 2; j <= nP; ++j)
						eval_labels[j][cf->gates[4*i+2]] = GTM[GTX_idx(ands, index, j)];
					mask_input[cf->gates[4*i+2]] = GTv[ands][index];
					for(int j = 2; j <= nP; ++j) {
						Hash(H, eval_labels[j][cf->gates[4*i]], eval_labels[j][cf->gates[4*i+1]], ands, index);
						xorBlocks_arr(H, H, &GT[GT_idx(ands, j, index, 0)], nP+1);
						for(int k = 2; k <= nP; ++k)
							eval_labels[k][cf->gates[4*i+2]] = H[k] ^ eval_labels[k][cf->gates[4*i+2]];
					
						block t0 = GTK[GTX_idx(ands, index, j)] ^ Delta;

						if(cmpBlock(&H[1], &GTK[GTX_idx(ands, index, j)], 1))
	...
	void Hash(block * H, const block & a, const block & b, uint64_t idx) {
		...
		for(int j = 0; j < 4; ++j) for(int i = 1; i <= nP; ++i) {
			H[j*(nP+1) + i] = H[j*(nP+1) + 0] ^ makeBlock(4*idx+j, i);
		}
		for(int j = 0; j < 4; ++j) {
			prp.permute_block(&H[j*(nP+1) + 1], nP);
		}
	}
	...
	void online (FlexIn<TP, IO> * input, FlexOut<TP, IO> *output) {
		bool * mask_input = new bool[cf->num_wire];
		...
		if(party!= 1) {
			...
			for(int i = 0; i < cf->num_gate; ++i) {
				if (cf->gates[4*i+3] == XOR_GATE) {
					...
					mask_input[cf->gates[4*i+2]] = GTv[ands][index];
					for(int j = 2; j <= nP; ++j) {
						Hash((block*)H, eval_labels[j][cf->gates[4*i]], eval_labels[j][cf->gates[4*i+1]], ands, index);
						xorBlocks_arr(H, H, &GT[GT_idx(ands, j, index, 0)], nP+1);
						for(int k = 2; k <= nP; ++k)
							eval_labels[k][cf->gates[4*i+2]] = H[k] ^ eval_labels[k][cf->gates[4*i+2]];
